...
public class Move extends jbt.execution.task.leaf.action.ExecutionAction {
	...
	protected void internalSpawn() {
		...
		getContext().setVariable("t", 3+System.currentTimeMillis()%3);
		getContext().setVariable("c", 0);
	...
	protected jbt.execution.core.ExecutionTask.Status internalTick() {
		 ...
		 */
		long t = Long.valueOf(getContext().getVariable("t").toString());
		long c = Long.valueOf(getContext().getVariable("c").toString());		
		c++;
		boolean finish = c >= t;
		getContext().setVariable("c", c);
		System.out.println(this.getClass().getCanonicalName() + " tick("+c+"/"+t+")");
		if(finish) {
			getContext().setVariable("danger", true);
