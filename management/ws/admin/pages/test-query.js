var md = angular.module('module_test-query', ['ng.ui', 'ng.ext.uploadify',
    'ng.ext.filter-grid','ng.ext.uploadifive',
    'ng.ext.tree']);
md.controller('MainCtrl', function ($scope, uiLog, uiTips, uiValid) {
    // ueditor
    var ue = UE.getEditor('test-editor', {serverUrl: '/a/m/ue/manager-test', zIndex: 900});
    setTimeout(function(){
        ue.setContent('<p style="color: red;">test</p>');
    }, 1000 * 2);

    $scope.showEditorContent = function () {
        var content = ue.getContent();
        uiTips.alert(content);
    };
...
//		$scope.dropdownTest2 = 10;
    ...
    $scope.next = function () {
        if (!uiValid.checkForm($scope.gridForm0) || !$scope.gridForm0.$valid) {
    ...
    /*uploadifive*/
    $scope.uploadImporttest = function (file, json, response) {
        console.log(json);
    };
});