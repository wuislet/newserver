        ...
        return {
            ...
            checkFormWithVal: function ($form, returnRequiredModel, $index) {
                ...
                var isSelectNull = function (one, val) {
                    return '? object:null ?' == val && one.is('select');
                };
        ...
        function ($compile, $parse, safeApply, log, tips) {
            return {
                ...
                link: function (scope, el, attrs) {
                    ...
                    // trigger first
                    if (opts.targetIndexModel) {
                        scope.$watch(opts.targetIndexModel, function (targetIndex) {
                            navs.findAll('a').eq(targetIndex || 0).trigger('click');
                        });
                    } else {
                    ...
                    for (; i < arr.length; i++) {
                        ...
                        for (; j < modelArr.length; j++) {
                            ...
                            scope.$watch(targetModelName, function () {
                                 ...
                                 */
                                if ((valid.filterWatchValid && valid.filterWatchValid(ctrl, attrs)) || !valid.filterWatchValid) {
                                    // valid again
    ...
    md.factory('uiPager', ['conf', function (conf) {
        return {
            ...
            create: function (rawData) {
                return {pageNum: rawData.pageNum, pageSize: rawData.pageSize, totalCount: rawData.totalCount};
            }
        };
    }]);
        ...
        return {
            ...
            link: function ($scope, el, attrs) {
                ...
                $scope.$watch('list', function (list) {
                    ...
                    var data = {
                        columns: $scope.columns, list: list,
